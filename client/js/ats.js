...
export function scoreResume(data) {
    ...
    const hasQuantified = data.experience.some(job => job.description && /[%0-9]/.test(job.description) ||
...
document.addEventListener('DOMContentLoaded', () => {
    const updateMiniUi = (result) => {
        ...
        if (result.score >= 80)
            colorCls = 'score-green';
        else if (result.score >= 50)
            colorCls = 'score-yellow';
        circle.className = `ats-score-circle ${colorCls} shadow-sm`;
        circle.innerHTML = result.score.toString();
        if (result.score >= 80)
            msg.innerText = "Looking good!";
        else if (result.score >= 50)
            msg.innerText = "Needs improvement";
        else
            msg.innerText = "Major issues found";
    };
    ...
    checkAtsBtn === null || checkAtsBtn === void 0 ? void 0 : checkAtsBtn.addEventListener('click', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            const saveRes = yield saveResp.json();
            if (!saveRes.success) {
            ...
            if (modalCircle && modalValue && modalTitle && rulesList) {
                ...
                if (finalScore >= 80) {
                    colorCls = 'score-green';
                    modalTitle.innerText = "Excellent ATS Compatibility";
                }
                else if (finalScore >= 50) {
                    colorCls = 'score-yellow';
                    modalTitle.innerText = "Average ATS Compatibility";
                }
                else {
          ...
          `;
            ...
            console.log(e);
            const { showToast } = yield import('./form.js');
